#!/usr/bin/env Rscript
# Render the maximal-substitution table as table_latex/tbl_fish_subs.tex.
#
#   Rscript code/make_tex.R
#
# Reads full precision from RAW_DIR, not the 4-decimal output/tbl_fish_subs.csv, so the
...
out_tex <- Sys.getenv("OUT_TEX", "table_latex/tbl_fish_subs.tex")
...
tex <- c(
  ...
  paste0("\\textit{Note}: The numbers in the table show the value of (\\ref{eq:dist.2}) ",
         "for each $j, l \\in \\{1,2,3,4\\}$ s.t. $j\\neq l$.  Alternative numbers 1, 2, 3, 4 ",
         "denote beach, boat, charter, pier, respectively.  All numbers are truncated to ",
