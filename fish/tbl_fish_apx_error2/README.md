# tbl:fish_apx_error2 — Approximation errors to a half-half mixture (fish, with fixed effects)

Paper: online appendix `sec:fish_with_fixed`.
...
Four steps, in order. Run everything from this directory.
    ...
    bash code/submit_em.sh fish 1 1-5             # degree 1, replicates 1-5; repeat with 2
    Rscript code/collect.R                        # -> output/tbl_fish_apx_error2.csv
    Rscript code/make_tex.R                       # -> table_latex/tbl_fish_apx_error2.tex
...
It takes about four minutes over a complete set.
...
degrees together are roughly 111,000 greedy tasks and 22,000 EM tasks. Degree-2 greedy tasks
are the long ones, measured at 2 to 21 hours each.
...
- **Dataset**: fishing-site choice, `data/fish/fish_J4_K2.csv` — price and catch rate
  averaged over the 1182 individuals, so |J| = 4 and K = 2. Alternatives are 1 = beach,
  2 = boat, 3 = charter, 4 = pier.
...
Greedy runs 1000 iterations, fixed at the call site in `approx_rum_GREEDY_hhmix_fish.py`.
...
Greedy writes one file per grid point:
    ...
    output/raw/Fish_hh_gd_<case>_<degree>_<gridindex>.txt
...
EM writes one CSV per chunk, under one directory per replicate:
    ...
    output/raw_em_tight/rep<N>/hh_Fish_<case>d<degree>_EM_error_<start>_<end>.csv
...
`collect.R` writes `output/tbl_fish_apx_error2.csv`: one row per case, with `linear_greedy`,
...
- greedy: `code/approx_rum_GREEDY_hhmix_fish.py` (argv: to-do index, case, degree)
- EM: `code/approx_rum_EM_hhmix_fish.py` (argv: chunk index, case, degree)

Both take an optional fourth argument giving the data file, defaulting to `FISH_DATA`.
