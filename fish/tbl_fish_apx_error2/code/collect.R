#!/usr/bin/env Rscript
# Aggregate the per-grid-point greedy and EM results into tbl:fish_apx_error2.
#
#   Rscript code/collect.R [raw_dir] [out_csv]
#
# raw_dir holds greedy/ and em_tight/rep<N>/, as in
# $SCRATCH/RUM_replication/results_final/fish/tbl_fish_apx_error2. The exhibit's own
# output/{raw,raw_em_tight} layout is accepted too. GREEDY_DIR and EM_ROOT override either
# half independently, for the case where the two live under different trees.
#
...
#
# Replicates that hold no files are skipped, and replicates that are byte-for-byte repeats
# of an earlier one are counted once: before the 2026-08-16 RNG fix every replicate drew
# the same initial points, so a directory count overstates the number of EM starts. The
# reported start count is the number of *distinct* replicates.

args    <- commandArgs(trailingOnly = TRUE)
raw_dir <- if (length(args) >= 1) args[1] else "output/raw"
out_csv <- if (length(args) >= 2) args[2] else "output/tbl_fish_apx_error2.csv"
...
# case -> (pi, pi-bar), from the if/elif ladder in both solvers. Alternatives are
...
greedy_dir <- Sys.getenv("GREEDY_DIR", "")
if (!nzchar(greedy_dir))
  greedy_dir <- pick(file.path(raw_dir, "greedy"), file.path(raw_dir, "raw"), raw_dir)
em_root <- Sys.getenv("EM_ROOT", "")
if (!nzchar(em_root))
  em_root <- pick(file.path(raw_dir, "em_tight"), file.path(raw_dir, "raw_em_tight"))

em_reps <- if (is.na(em_root) || !dir.exists(em_root)) character(0) else
           sort(list.dirs(em_root, recursive = FALSE))
# A replicate directory that was created but never written to would otherwise inflate the
# coverage denominator by a full grid.
em_reps <- em_reps[vapply(em_reps, function(d) length(list.files(d)) > 0, logical(1))]

if (is.na(greedy_dir)) stop("no greedy directory under ", raw_dir)
if (!length(em_reps))  stop("no non-empty em_tight replicate directories under ", raw_dir)
...
read_greedy <- function(case, degree) {
  scan_files(greedy_dir, sprintf("Fish_hh_gd_%d_%d_*.txt", case, degree), "FNR==5")
}

# One numeric vector per replicate, so duplicates can be identified before they are pooled.
read_em_by_rep <- function(case, degree) {
  lapply(em_reps, function(rp)
    scan_files(rp, sprintf("hh_Fish_%dd%d_EM_error_*.csv", case, degree),
               "FNR>1{print $2}", fs = ","))
}

cat(sprintf("greedy dir    %s\n", greedy_dir))
cat(sprintf("em root       %s\n", em_root))
...
for (case in CASES) {
  ...
  for (degree in DEGREES) {
    ...
    ev <- read_em_by_rep(case, degree)
    nonempty <- ev[lengths(ev) > 0]
    n_distinct <- length(unique(lapply(nonempty, function(v) paste(v, collapse = " "))))
    e <- unlist(ev, use.names = FALSE)
...
if (any(cov$em_starts < length(em_reps)))
  cat(sprintf("note: %d replicate directories, but only %d distinct EM start(s) -- ",
              length(em_reps), max(cov$em_starts)),
      "the repeats are byte-identical and add nothing to the minimum\n", sep = "")
...
printed <- tab
printed[val_cols] <- lapply(tab[val_cols], function(x)
  ifelse(is.na(x), "---", sprintf("%.3f", trunc3(x))))

cat("\n=== tbl:fish_apx_error2 (truncated at three decimals) ===\n")
