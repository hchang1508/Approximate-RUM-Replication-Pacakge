...
#
# Input : output/raw/Fish_hh_gd_<case>_<d>_<gridindex>.txt   (one file per finished cell)
...
files <- list.files(RAW_DIR, pattern = "^Fish_hh_gd_[0-9]+_[0-9]+_[0-9]+\\.txt$")
cat(sprintf("raw dir  : %s   (%d finished files)\n", RAW_DIR, length(files)))
cat(sprintf("todo dir : %s\n", TODO_DIR))
cat(sprintf("grid     : 0..%d  (%d points)\n\n", GRID_MAX, GRID_MAX + 1))

# Fish_hh_gd_<case>_<d>_<gridindex>.txt
parts <- regmatches(files, regexec("^Fish_hh_gd_([0-9]+)_([0-9]+)_([0-9]+)\\.txt$", files))
done <- data.frame(
