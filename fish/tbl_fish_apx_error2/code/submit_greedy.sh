#!/bin/bash
# Greedy algorithm for tbl:fish_apx_error2 (fish, fixed-effects grid).
#
#   cd RUM_replication/fish/tbl_fish_apx_error2 && bash code/submit_greedy.sh <case> <degree>
...
#
# Writes output/raw/Fish_hh_gd_<case>_<degree>_<gridindex>.txt. Aggregate with code/collect.R.
...
cd /home/hc654/RUM_replication/fish/tbl_fish_apx_error2
...
for c in $CASES; do
  ...
  CMD=(sbatch
    ...
    --export=ALL,FISH_DATA=${FISH_DATA},TODO_DIR=${TODO_DIR},OUT_DIR=${OUT_DIR}
    --wrap="$PYTHON -u code/approx_rum_GREEDY_hhmix_fish.py \$SLURM_ARRAY_TASK_ID ${c} ${DEGREE}"
  )
