#!/usr/bin/env Rscript
# Aggregate the per-task greedy and EM results into tbl:fish_apx_error1.
#
#   Rscript code/collect.R [raw_dir] [out_csv]
#
# raw_dir is either a single directory holding Fish_gd_<order>_<index>.txt and
# Fish_em_<order>_<index>.txt, or a parent holding greedy/ and em_tight/. Each file holds
# five numbers: pi(0), pi(1), pi(2), pi(3), raw_error.
#
...
out_csv <- if (length(args) >= 2) args[2] else "output/tbl_fish_apx_error1.csv"

N_ALT <- 4
N_RANK <- factorial(N_ALT)
COUNT  <- choose(N_ALT, 2) + choose(N_ALT, 3) + 1        # 11 = |cal_D|; no singleton menus
CARD_D <- COUNT                                           # |D| = count: singletons unused
FACTOR <- COUNT / sqrt(CARD_D)                            # = sqrt(11) = 3.3166

cat(sprintf("code divisor  count = |cal_D| = %d  (pairs %d + triples %d + full set 1; no singletons)\n",
            COUNT, choose(N_ALT, 2), choose(N_ALT, 3)))
cat(sprintf("paper norm    |D| = count = %d  (singleton menus are never evaluated)\n", CARD_D))
...
read_one <- function(alg, order, index) {
  f <- file.path(alg_dir(alg), sprintf("Fish_%s_%d_%d.txt", alg, order, index))
...
lab <- function(r) paste(r + 1, collapse = ">")   # 0-based -> the paper's 1-based labels
...
factor_primary <- FACTOR   # sqrt(11); the only scaling this package uses
tab_scaled <- tab
tab_scaled[val_cols] <- tab[val_cols] * factor_primary

# A ranking is linearly representable exactly when the linear specification attains 0.
