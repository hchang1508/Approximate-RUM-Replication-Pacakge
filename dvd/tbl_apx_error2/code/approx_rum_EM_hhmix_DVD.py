...
def _n_starts(default=10):
    """Random initialisations of EM per grid point, taken inside one task."""
...
def _standardise():
    """Whether to centre and scale the feature columns."""
    return os.environ.get('EM_STANDARDIZE', '0') not in ('0', '', 'false', 'False')

def _chunk(default=300):
    """Grid points per array task."""
    return int(os.environ.get('EM_CHUNK', default))

warnings.simplefilter('ignore')

# Generator behind every EM initial point, reseeded from the run's derived seed in
# __main__. np.random.seed() drives the legacy global RNG and does not touch a Generator,
# so seeding it here is what makes SEED_BASE reach the initial points at all; without it
# separate replicate processes all draw the same start. EM_RNG_SEED=0 restores the
# pre-2026-08-16 behaviour. See README.md.
...
if __name__ == "__main__":
    ...
    output_name=_out_dir() + '/hh_DVD_'+str(case)+'d'+str(d)+'_EM_error_'+str(index_start)+'_'+str(index_end)+'.csv'
