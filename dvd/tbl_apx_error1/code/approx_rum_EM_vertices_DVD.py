"""Approximation error of a mixed-logit model to rho^pi, by EM (DVD, no fixed effects).

Usage:
    python approx_rum_EM_vertices_DVD.py <index> <order> [data.csv]

    index   0-23, position in permutations((0, 1, 2, 3))
    order   1 for linear in characteristics, 2 for quadratic

Writes OUT_DIR/DVD_em_<order>_<index>.txt holding pi(0..3) followed by the
approximation error, minimised over EM_STARTS random initial points.

Environment: OUT_DIR, DVD_DATA, SEED_BASE, EM_STARTS, EM_MIXTURE, EM_MAX_ITER,
...
def _standardise():
    ...
    return os.environ.get('EM_STANDARDIZE', '1') not in ('0', '', 'false', 'False')

def _max_iter(default=1000):
    """Iteration cap per EM fit."""
    return int(os.environ.get('EM_MAX_ITER', default))

def _mixture(default=18):
    """Number of mixture components M."""
    return int(os.environ.get('EM_MIXTURE', default))

_SEED_BASE = int(os.environ.get('SEED_BASE', 20240808))
_SEED = _SEED_BASE + 1000 * order + index
...
if __name__ == "__main__":
    ...
    num_init_choice = int(os.environ.get('EM_STARTS', 10))
    
    output = []
    features = _load_features('../../data/dvd/dvd_J4_K2.csv')
...
#####################################################
    ...
    file_name = _out_dir() + '/DVD_em_' + str(order) + '_' + str(index) + '.txt'
    np.savetxt(file_name, np.array([pref[0],pref[1],pref[2],pref[3],np.nanmin(np.asarray(output, dtype=float))]))
